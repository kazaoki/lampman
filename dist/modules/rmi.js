        ...
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    ...
                    lines = child.execFileSync('docker', ['images', '--format={{.Repository}}\t{{.Tag}}\t{{.ID}}\t{{.Size}}\t{{.CreatedSince}}']).toString().split(/\r?\n/);
                    list = [];
                    for (_i = 0, lines_1 = lines; _i < lines_1.length; _i++) {
                        ...
                        if (name_1.length) {
                            list.push({
                                title: "[IMAGE] " + name_1 + ":" + tag + " (" + id + ") " + size,
                                value: id,
                                description: since,
                            });
                        }
                ...
                case 1:
                    response = _a.sent();
                    child.spawnSync('docker', __spreadArrays(['rmi', '-f'], response.targets), { stdio: 'inherit' });
                    return [2];
            }
        });
    });
